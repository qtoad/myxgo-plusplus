...
func (s *WechatpayClient) UnifiedOrder(
	...
	//发起post请求
	unifiedOrderUrl := "https://api.wechatpay.weixin.qq.com/pay/unifiedorder"
	...
	if err := util.FromXml(respData, unifiedOrderResponse); err != nil {
		return nil, err
	} else {
...
func (s *WechatpayClient) GetUnifiedOrderXml(
	...
	timeStartString := util.TimeToString(nowDate, "20060102150405")
	timeExpireString := util.TimeToString(expiredate, "20060102150405")
	...
	unifiedOrderRequest.NonceStr = uuid.UUID4().String()
...
func (s *WechatpayClient) GetUnifiedOrderResult(
	...
	unifiedOrderResult.NonceStr = uuid.UUID4().String()
	unifiedOrderResult.Timestamp = fmt.Sprintf("%d", util.UnixTimestamp())
...
func (s *WechatpayClient) GetNotifyResult(httpRequest *http.Request) (*PayResultNotify, error) {
	...
	if err := util.FromXml(string(rawBody), resultNotify); err != nil {
		return nil, err
	}
