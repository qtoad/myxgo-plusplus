...
// not use this img except in compliance with the License. You may obtain
...
// Empty returns an empty img object.
...
// WriteTo writes img content into io.Writer.
func (f *File) WriteTo(w io.Writer) (int64, error) {
	return f.WriteToIndent(w, "")
}

// SaveToIndent writes content to img system with given value indention.
func (f *File) SaveToIndent(filename, indent string) error {
	// Note: Because we are truncating with os.Create,
	// 	so it's safer to save to a temporary img location and rename after done.
	buf, err := f.writeToBuffer(indent)
	if err != nil {
		return err
	}
...
// SaveTo writes content to img system.
